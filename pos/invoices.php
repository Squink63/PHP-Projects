...
<div class="sidebar">
  ...
  <a  href="admin.php">Home</a>
    ...
    <?php if( $user_in_s['role_id'] == 1 || in_array('sr', $selectedPermissions)) : ?>
        <a class="active" href="invoices.php">Suppliers</a>
    <?php endif ?>
...
<div class="backdrop"></div>
    <nav class="navbar">
        <div class="container-fluid">
            <?php if( $user_in_s['role_id'] == 1 || in_array('sc', $selectedPermissions)) : ?>
              <button id="addr" class="navbar-brand text-dark">Add Supplier</button>
            <?php else : ?>
    ...
    <?php
      ...
      $start = 0;
      $row_per_page = 5;
      $statement = $db->query("SELECT * FROM suppliers");
      ...
      $pages =  ceil($num_of_rows / $row_per_page);
        ...
        $result = $db -> query("SELECT * FROM suppliers LIMIT $start, $row_per_page");
...
<table class="table table-dark">
      ...
      <tbody>
        ...
        <?php foreach($users as $user) : ?>
          ...
          <td>
            <div class="btn-group">
                <?php if( $user_in_s['role_id'] == 1 || in_array('su', $selectedPermissions)) : ?>
                  <a href="#" class="btn btn-sm btn-success me-3">Edit</a>
                <?php endif ?>

                <?php if( $user_in_s['role_id'] == 1 || in_array('sd', $selectedPermissions)) : ?>
                  <a href="delsup.php?id=<?= $user['id'] ?>" class="btn btn-sm btn-danger">Delete</a>
                <?php endif ?>
            </div>
          </td>
        </tr>
    ...
    <div id="cf" class="container p-3">
      <form action="addsup.php" method="post" id="fo">
              ...
              <div class="row mb-3">
                ...
                <div class="col-sm-10">
                  <input type="text" name="phone" class="form-control" id="colFormLabelSm" required>
                </div>
              </div>


              
              <button class="btn btn-primary w-100 my-4">Add Supplier</button>
      </form>
    </div>
